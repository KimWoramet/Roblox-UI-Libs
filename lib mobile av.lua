...
local Settings = krukoihub.Settings
local Flags = krukoihub.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(krukoihub.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then krukoihub.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then krukoihub.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then krukoihub.Save["Theme"] = decode["Theme"] end
			end
		end
	end
	
	pcall(Save, "krukoihublibrary.json")
end
...
local Connections, Connection = {}, krukoihub.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(krukoihub.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = krukoihub.Themes[krukoihub.Save.Theme]

local function AddEle(Name, Func)
	krukoihub.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = krukoihub.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function krukoihub:GetIcon(index)
...
function krukoihub:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	krukoihub.Save.Theme = NewTheme
	SaveJson("krukoihublibrary.json", krukoihub.Save)
	Theme = krukoihub.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(krukoihub.Instances, function(_,Val)
...
function krukoihub:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function krukoihub:MakeWindow(Configs)
	local WTitle = Configs[1] or Configs.Name or Configs.Title or "krukoihublibrary"
	local WMiniText = Configs[2] or Configs.SubTitle or "by : kimmybabyboyy"
	...
	local UISizeX, UISizeY = unpack(krukoihub.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, krukoihub.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			krukoihub.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("krukoihublibrary.json", krukoihub.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		krukoihub.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("krukoihublibrary.json", krukoihub.Save)
	end)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			krukoihub.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(krukoihub.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(krukoihub.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(krukoihub.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(krukoihub.Options, {type = "Section", Name = SectionName, func = Section})
